...
class JTPoint(object):
    def __init__(self, point_number, num_points, now, duration, starts, joint_intervals): # formerly nsecs_interval instead of duration
        ...
        for i in range(len(starts)):
            self.point.positions.append(starts[i] + joint_intervals[i]*point_number) # [start + joint_interval*point_number, 0, 0, 0, 0, 0]
        #TODO: Fix "not strictly increasing in time" & "dropping first trajectory point" errors (first point refers to current position, so maybe it's not so important)
        #added_secs = int((now.nsecs + float(duration[1]) * point_number / (num_points-1)) / 1000000000.0) # this nsecs_interval is a concise spacing between point times. Now we want to divide these from total.
        #self.point.time_from_start.set(now.secs + added_secs + float(duration[0]) * point_number / (num_points-1), 
                                       #(now.nsecs + float(duration[1]) * point_number / (num_points-1)) % 1000000000)
...
def set_joint_positions(): # formerly set_angle()
    ...
    while not rospy.is_shutdown():
        rospy.init_node('motion_controller2', anonymous=False)
        pub1 = rospy.Publisher('/leg_l_controller/command', trajectory_msgs.msg.JointTrajectory, queue_size=0)
        pub2 = rospy.Publisher('/leg_r_controller/command', trajectory_msgs.msg.JointTrajectory, queue_size=0)
        pub3 = rospy.Publisher('/toes_l_controller/command', trajectory_msgs.msg.JointTrajectory, queue_size=0)
        pub4 = rospy.Publisher('/toes_r_controller/command', trajectory_msgs.msg.JointTrajectory, queue_size=0)
        ...
        '''
        mgpi.go_to_joint_state(mgpi.leg_l_group, goal[0]) # may need to be mgpi.leg_group instead of 'leg_l_manipulator'
        mgpi.go_to_joint_state(mgpi.leg_r_group, goal[1])
        mgpi.go_to_joint_state(mgpi.toes_l_group, goal[2])
        mgpi.go_to_joint_state(mgpi.toes_r_group, goal[3])
        mgpi.go_to_joint_state(mgpi.spine_group, goal[4])
        '''
        # Assumes that previous goal has been met and has not deviated since
        
        #hip_r_x_start = starting_angles[0][0]
        #hip_r_x_end = target_angles[0][0]
        ...
        '''
        # point_number, now, nsecs_interval, start, joint_interval
        leg_l_point0 = JTPoint(0, num_points, now, duration, leg_l_starts, leg_l_joint_intervals)
        leg_l_point1 = JTPoint(1, num_points, now, duration, leg_l_starts, leg_l_joint_intervals)
        leg_l_point2 = JTPoint(2, num_points, now, duration, leg_l_starts, leg_l_joint_intervals)
        
        #now = rospy.Time.now()
        leg_r_point0 = JTPoint(0, num_points, now, duration, leg_r_starts, leg_r_joint_intervals)
        leg_r_point1 = JTPoint(1, num_points, now, duration, leg_r_starts, leg_r_joint_intervals)
        leg_r_point2 = JTPoint(2, num_points, now, duration, leg_r_starts, leg_r_joint_intervals)
        
        #now = rospy.Time.now()
        ball_l_point0 = JTPoint(0, num_points, now, duration, ball_l_starts, ball_l_joint_intervals)
        ball_l_point1 = JTPoint(1, num_points, now, duration, ball_l_starts, ball_l_joint_intervals)
        ball_l_point2 = JTPoint(2, num_points, now, duration, ball_l_starts, ball_l_joint_intervals)
        
        #now = rospy.Time.now()
        ball_r_point0 = JTPoint(0, num_points, now, duration, ball_r_starts, ball_r_joint_intervals)
        ball_r_point1 = JTPoint(1, num_points, now, duration, ball_r_starts, ball_r_joint_intervals)
        ball_r_point2 = JTPoint(2, num_points, now, duration, ball_r_starts, ball_r_joint_intervals)
        
        #now = rospy.Time.now()
        spine_point0 = JTPoint(0, num_points, now, duration, spine_starts, spine_joint_intervals)
        spine_point1 = JTPoint(1, num_points, now, duration, spine_starts, spine_joint_intervals)
        spine_point2 = JTPoint(2, num_points, now, duration, spine_starts, spine_joint_intervals)
        '''
        '''
        points.effort = [20, 20, 20, 20, 20, 20]
        points.velocities = [20, 20, 20, 20, 20, 20]
        points.accelerations = [1, 1, 1, 1, 1, 1]
        '''
        #leg_r_jt.points = [point0, point1, point2, point3, point4, point5]  
        now.secs = 0
        now.nsecs = 0
        ...
        client.send_message('rpy:{}:{}:{} y_dist:{}'.format('dummy', 'dummy', 'dummy', min_distance)) # may want to change to y_position
        ...
        fallen = '{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}'.format(fallen_status,
                  ...
                  hip_l_x, hip_l_y, knee_l, ankle_l_y, ankle_l_x, hip_r_x, hip_r_y, knee_r, ankle_r_y, ankle_r_x)
